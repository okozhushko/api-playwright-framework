...
- `tests.yml`'s PR gate also runs against that same live API — deliberately, not by oversight. This suite verifies real integration behavior against JSONPlaceholder specifically (including its quirks, e.g. `PUT` on a missing id returning `500`), so a mock/stub layer would only test the client against its own author's assumptions. A transient external outage blocking a merge is an accepted trade-off of that design, not a bug to engineer around with speculative test-double infrastructure.
